...
namespace Reversi
{
	...
	public class Board
	{
        ...
        public bool IsValidMoveTwo(int color, int row, int col)
        {
            // The square must be empty.
            if (this.squares[row, col] != Board.Empty)
                return false;

            // Must be adjacent to any piece
            int dr, dc;
            for (dr = -1; dr <= 1; dr++)
                for (dc = -1; dc <= 1; dc++)
                    if (!(dr == 0 && dc == 0) && this.squares[row, col] == Board.Empty)
                        return true;
        ...
        public bool GetValidMoveCountTwo()
        {
            return false;
        }
